...
def test_no_days_before_min_date(seed_random):
    has_results = False
    for cpr in MonthGenerator(2007, Options(months=[3], days=[1, 2], min_date=date(2007, 3, 2))):
        has_results = True
        assert cpr.day == 2
    assert has_results
...
def test_min_days_only_affect_correct_month(seed_random):
    has_results = False
    for cpr in MonthGenerator(2002, Options(months=[4, 5], days=[10], min_date=date(2002, 4, 20))):
        has_results = True
        assert cpr.month == 5
    assert has_results
...
def test_min_date_choise_filter(seed_random):
    with pytest.raises(StopIteration):
        cpr = next(iter(MonthGenerator(2002, Options(months=[4, 5], days=[10], min_date=date(2002, 5, 20)))))
        print(repr(cpr))
        

def test_min_days_only_affect_correct_year(seed_random):
    next(iter(MonthGenerator(1995, Options(months=[7], days=[5], min_date=date(1994, 7, 10)))))

def test_no_days_after_max_date(seed_random):
    has_results = False
    for cpr in MonthGenerator(2007, Options(months=[3], days=[1, 2], max_date=date(2007, 3, 1))):
        has_results = True
        assert cpr.day == 1
    assert has_results
...
def test_max_days_only_affect_correct_month(seed_random):
    has_results = False
    for cpr in MonthGenerator(2002, Options(months=[4, 5], days=[10], max_date=date(2002, 5, 5))):
        has_results = True
        assert cpr.month == 4
    assert has_results
...
def test_max_date_choise_filter(seed_random):
    with pytest.raises(StopIteration):
        cpr = next(iter(MonthGenerator(2002, Options(months=[4, 5], days=[10], max_date=date(2002, 4, 5)))))
        print(repr(cpr))

def test_max_day_only_affect_coorect_year(seed_random):
    next(iter(MonthGenerator(1995, Options(months=[7], days=[5], max_date=date(1994, 7, 10)))))
...
def test_min_and_max_date(seed_random):
    ...
    seen_month_2 = False
    seen_month_3 = False
    for cpr in MonthGenerator(1955, options=Options(months=[10, 11, 12], days=[10,20], min_date=min_date, max_date=max_date)):
