...
def main() -> CprBuilder:
    ...
    if parsed_args.year is not None:
        list_parser(parsed_args.year, builder.with_year, builder.with_year_range)

    if parsed_args.month is not None:
        list_parser(parsed_args.month, builder.with_month, builder.with_month_range)
...
def list_parser(argument: str, single_func: Callable[[int], Any], range_func: Callable[[int, int], Any]) -> None:
